...
public:
    residual(const SymmetryMatrix::Operator &hamiltonian, bool subtract_Energy, SymmetryMatrix::Operator *Q = nullptr)
            : m_hamiltonian(
...
public:
    Presidual(const SymmetryMatrix::Operator &hamiltonian, const std::vector<Pvector> &P) : m_hamiltonian(hamiltonian),
                                                                                            m_P(P) { }
...
public:
    meanfield_residual(const SymmetryMatrix::Operator &hamiltonian, bool subtract_Energy,
                       SymmetryMatrix::Operator *Q = nullptr)
...
std::vector<double>
Run::DIIS(const SymmetryMatrix::Operator &ham, const State &prototype, double energyThreshold, int maxIterations) {
...
//  profiler->start("Davidson preamble");
    ...
    auto initialNP =
            std::min(std::max(options.parameter("PSPACE_INITIAL", nState), nState),
                     static_cast<int>(ww.front().size()));
    auto maxNP =
    ...
    for (int n = 0; n < maxIterations; n++) {
        ...
        for (int i = 0; i <= n; i++) {
            for (int j = 0; j <= n; j++) {
                ...
                for (int k = 0; k <= n; k++) {
                    if (k != track)
                        hinvij += eigenvectors[i + k * (n + 1)] * eigenvectors[j + k * (n + 1)] /
                                  (eigenvalues[k] - energy);
                }
...
//    xout << "l2norm="<<l2norm<<" "<<w*w<<std::endl;
        ...
        double mu = d2Edmu2 == (double) 0 ? (double) 0 : sqrt(2 * energyThreshold / d2Edmu2)
                                                         * options.parameter("PENALTY_SCALE",
                                                                             std::vector<double>(1, 1)).at(0);
...
//      w += factor*g;
...

        {
            ...
            for (size_t i = 0; i < nhist; i++) {
                ...
                if (fcumulative[i] > 1 - 1e-8 ||
                    (edges[i] < 1e-4 && i > 0 && fcumulative[i] == fcumulative[i - 1]))
                    break;
            }
#ifdef MOLPRO
...
gci::projector(const SymmetryMatrix::Operator &source, const std::string special, const bool forceSpinUnrestricted) {
        ...
        for (size_t oi = 0; oi < hamiltonian.dimension(si, 0, spini > 0); oi++) {
            ...
            for (auto sj = 0; sj < 8; sj++) {
                for (size_t oj = 0; oj < hamiltonian.dimension(sj, 0, spinj > 0); oj++) {
                    result(j++, i) = hamiltonian.O2(spini > 0, spinj > 0).smat(0, si, oi, oi)->block(sj)[
                            (oj + 2) * (oj + 1) / 2 - 1];
                }
            }
            i++;
                            ...
                            ((si < sj) ?
                             ...
                             ((si > sj) ?
                              hamiltonian.O2(spin > 0, spin > 0).smat(si ^ sj, sj, oj, oi)->blockMap(sj)(oj, oi)
                                        : ((i > j) ?
                                           hamiltonian.O2(spin > 0, spin > 0).smat(0, si, oi, oj)->block(si)[
                                                   (oi * (oi + 1)) / 2 + oj]
                                                   :
                                           hamiltonian.O2(spin > 0, spin > 0).smat(0, sj, oj, oi)->block(sj)[
                                                   (oj * (oj + 1)) / 2 + oi]
                              )));
                }
            }
...
//      xout << "o: ijkl "<<oi<<oj<<ok<<ol<<std::endl;
            ...
            if (type == FCIdump::I2aa) {
                (sij ? integrals_aa.smat(sij, si, oi, oj)->blockMap(sk)(ok, ol) : integrals_aa.smat(sij, si, oi,
                                                                                                    oj)->block(sk)[
                        ok * (ok + 1) / 2 + ol]) = value;
                (sij ? integrals_aa.smat(sij, sk, ok, ol)->blockMap(si)(oi, oj) : integrals_aa.smat(sij, sk, ok,
                                                                                                    ol)->block(si)[
                        oi * (oi + 1) / 2 + oj]) = value;
            } else if (type == FCIdump::I2ab) {
                (sij ? integrals_ab.smat(sij, si, oi, oj)->blockMap(sk)(ok, ol) : integrals_ab.smat(sij, si, oi,
                                                                                                    oj)->block(sk)[
                        ok * (ok + 1) / 2 + ol]) = value;
            } else if (type == FCIdump::I2bb) {
                (sij ? integrals_bb.smat(sij, si, oi, oj)->blockMap(sk)(ok, ol) : integrals_bb.smat(sij, si, oi,
                                                                                                    oj)->block(sk)[
                        ok * (ok + 1) / 2 + ol]) = value;
                (sij ? integrals_bb.smat(sij, sk, ok, ol)->blockMap(si)(oi, oj) : integrals_bb.smat(sij, sk, ok,
                                                                                                    ol)->block(si)[
                        oi * (oi + 1) / 2 + oj]) = value;
...
SymmetryMatrix::Operator gci::fockOperator(const SymmetryMatrix::Operator &hamiltonian, const Determinant &reference,
...
//    unsigned int oo = offset(o);
        for (unsigned int i = 1; i <= basisSize; i++) {
            for (unsigned int j = 1; j <= i; j++) {
                if (reference.orbitalSpace->orbital_symmetries[i - 1] !=
...
SymmetryMatrix::Operator
gci::sameSpinOperator(const SymmetryMatrix::Operator &hamiltonian, const Determinant &reference,
                      const std::string description) {
    SymmetryMatrix::dim_t dims;
    for (auto s = 0; s < 8; s++) {
        dims.push_back(hamiltonian.dimension(s, 0, 0));
