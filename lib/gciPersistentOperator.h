...
public:
     ...
     */
    PersistentOperator(const std::shared_ptr<SymmetryMatrix::Operator> &op, std::string _description, int root,
                       hid_t id, bool in_memory = false);
    ...
    std::shared_ptr<SymmetryMatrix::Operator> get() const;
    //! unique description of the operator
    std::string description();
protected:
    //! converts operator to bytestream and writes it to hdf5 file
    void store_bytestream(const std::shared_ptr<SymmetryMatrix::Operator> &op, int root);
};
