...
public:
  HProductSetF()
      : nMode(4), nModal(5), modeCouplingLevel(2), vibSpace(nMode, nModal, modeCouplingLevel), prodSet(vibSpace) {
    if (nMode < 2)
      throw std::logic_error("Fixture requires at least 2 modes");
    if (nModal < 5)
      throw std::logic_error("Fixture requires at least 4 modals");
  }
...
TEST_F(HProductSetF, generateFullSpace) {
  ...
  auto newEnd = std::copy_if(prodSet.begin(), prodSet.end(), uniqueCopy.begin(), [&](const HProduct &pIn) {
    int count = 0;
    for (const auto &el : prodSet)
      count += pIn == el ? 1 : 0;
    return count == 1;
  });
