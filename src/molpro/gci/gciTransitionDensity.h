 ...
 */
template <typename T, typename A = std::allocator<T>> class ctor_allocator : public A {
  using a_t = std::allocator_traits<A>;

public:
  using A::A; // Inherit constructors from A

  template <typename U> struct rebind { using other = ctor_allocator<U, typename a_t::template rebind_alloc<U>>; };
...
public:
  ...
  TransitionDensity(const Wavefunction &w, const StringSet::const_iterator &alphaStringsBegin,
                    ...
                    const StringSet::const_iterator &betaStringsEnd, molpro::parity_t parity, bool doAlpha = true,
                    bool doBeta = true);
