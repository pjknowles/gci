...
molpro::Operator constructOperatorT1(const molpro::FCIdump &dump) {
    ...
    if (rank == 0) {
        ...
        molpro::Operator result(molpro::dims_t{dim, dim, dim, dim}, 1,
...
MixedOperator::MixedOperator
        (const molpro::FCIdump &fcidump) : nMode(fcidump.parameter("NMODE", std::vector<int>{0})[0]),
...
//    std::cout << "MixedOperator: zpe = " << zpe << std::endl;
    ...
    auto store_fcidump = [&](const std::string &fname, const VibOp &vibOp) {
        if (file_exists(fname)) Hmix[vibOp.type].push_back(MixedOpTerm(vibOp, molpro::FCIdump(fname)));
    };
    if (m_inc_d1) {
