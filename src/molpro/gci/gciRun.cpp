...
std::vector<double> Run::run() {
  ...
  if (method == "RSPT") {
    ...
    //    xout <<"MP total energies" ; double totalEnergy=0; for (int i=0; i<(int)emp.size(); i++) xout <<"
    //    "<<(emp[i]=totalEnergy+=emp[i]); xout <<std::endl; energies.resize(1); energies[0] = totalEnergy;
...

  {
    ...
    if (profile > 1)
      xout << profiler->str(profile, false) << std::endl;
...
std::vector<double> Run::CSDavidson(const molpro::Operator &ham, const State &prototype, double energyThreshold,
  ...
  if (compressive)
    xout << "Compressive sampling algorithm, k=" << compressionK << ", l=" << compressionL
  ...
  for (int n = 0; n < maxIterations; n++) {
    ...
    // { xout << "Reduced hamiltonian:"<<std::endl; for (int i=0; i < n+1; i++) { for (int j=0; j < n+1; j++) xout <<"
...
continuumFound:
  ...
  for (int m = 2; m <= maxOrder; m++) {
    ...
    for (int j = 1; j < m; j++) {
      //                xout << "density"<<j<<m-j<<_IPT_c[j].density(1, true , true, &_IPT_c[m-j], "gamma
    ...
    //      xout << "density"<<0<<m-0<<_IPT_c[0].density(1, true , true, &_IPT_c[m-0], "gamma
...
void FCIDump(const molpro::Operator &op, const std::string filename, std::vector<int> orbital_symmetries) {
  ...
  if (verbosity > 0) {
    ...
    xout << "integral addresses " << &integrals_a.block(0)[0] << " " << &integrals_b.block(0)[0] << std::endl;
    xout << "integral addresses " << &integrals_aa << " " << &integrals_ab << " " << &integrals_bb << std::endl;
    //      xout << "integral sizes "<< integrals_aa.size()<<" "<<integrals_ab.size()<<"
      ...
      for (j = 1; j <= i; j++) {   // xout << "j="<<j<<std::endl;
        for (k = 1; k <= i; k++) { // xout << "k="<<k<<std::endl;
          for (l = 1; l <= (i == k ? j : k); l++) {
            ...
            //      xout << "s: ijkl "<<si<<sj<<sk<<sl<<std::endl;
            //      xout << "o: ijkl "<<oi<<oj<<ok<<ol<<std::endl;
            //      xout << (sij ? integrals_ab.smat(sij,si,oi,oj)->blockMap(sk)(ok,ol) :
            //      integrals_ab.smat(sij,si,oi,oj)->block(sk)[ok*(ok+1)/2+ol]) <<" "<< i << " "<<j<<" "<<k<<" "<<l
            //      <<std::endl;; xout << (sij ? &integrals_ab.smat(sij,si,oi,oj)->blockMap(sk)(ok,ol) :
            //      &integrals_ab.smat(sij,si,oi,oj)->block(sk)[ok*(ok+1)/2+ol]) <<" "<< i << " "<<j<<" "<<k<<" "<<l
            //      <<std::endl;;
