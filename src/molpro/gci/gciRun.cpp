...
struct residual {
 ...
 public:
  residual(const molpro::Operator &hamiltonian, bool subtract_Energy, molpro::Operator *Q = nullptr) : m_hamiltonian(
...
Run::Run(std::string fcidump)
    : m_hamiltonian(constructOperator(molpro::FCIdump(fcidump))) {
#ifdef HAVE_MPI_H
...
std::vector<double> Run::run() {
    if (profiler == nullptr)
        profiler = std::make_unique<molpro::Profiler>("GCI", molpro::Profiler::sortMethod::wall, INT_MAX, gci::mpi_comm_compute);
 ...
 */
static
molpro::Operator* projector(const molpro::Operator& source,
                                    std::string special,
...
std::vector<double> Run::DIIS(const molpro::Operator &ham, const State &prototype, double energyThreshold, int maxIterations) {
...
std::vector<double> Run::CSDavidson(const molpro::Operator &ham,
                                    const State &prototype,
                                    double energyThreshold, int nState, int maxIterations) {
...
std::vector<double> Run::RSPT(const std::vector<molpro::Operator *> &hams,
                              const State &prototype,
                              int maxOrder,
...
molpro::Operator* gci::projector(const molpro::Operator& source, const std::string special, const bool forceSpinUnrestricted) {
  molpro::dim_t dims;
  for (auto s=0; s<8; s++)
    dims.push_back(source.dimension(s,0,0));
  auto result = new molpro::Operator(dims,
                                             1,
...
molpro::Operator gci::fockOperator(const molpro::Operator& hamiltonian, const Determinant &reference, const std::string description) {
  molpro::dim_t dims;
  for (auto s=0; s<8; s++)
    dims.push_back(hamiltonian.dimension(s,0,0));
  molpro::Operator f(dims,
             1,
             hamiltonian.m_uhf,
...
molpro::Operator gci::sameSpinOperator(const molpro::Operator& hamiltonian, const Determinant &reference, const std::string description) {
  molpro::dim_t dims;
  for (auto s=0; s<8; s++)
    dims.push_back(hamiltonian.dimension(s,0,0));
  molpro::Operator result(dims,
                  hamiltonian.m_rank,
                  true,
                  hamiltonian.m_symmetry,
                  true,
