...
public:
   ...
   * \param State Some State object from which to copy number of electrons etc for bound checking, and to define orbital
   * symmetries \param Spin 1=alpha, -1=beta
   */
  explicit String(const State *State = nullptr, int Spin = 1);
  /*!
   ...
   * symmetries
   */
  explicit String(std::vector<char> bytestream, const State *State = nullptr);
  /*!
     \brief
  ...
  /*!
     ...
     \return int the phase change required to bring the determinant into canonical form before annihilation is returned
     (plus or minus 1), or else zero if the orbital was not present in the determinant.
    */
  int destroy(orbital_type orbital) {
    if (orbitalSpace == nullptr || orbital == (unsigned int)0 || orbital > (unsigned int)orbitalSpace->total())
      throw std::range_error("invalid orbital");
    if (m_orbitals.empty())
      return (int)0; // throw "too few electrons in String";
