...
void Wavefunction::operatorOnWavefunction(const molpro::Operator &h,
                    ...
                    if (NextTask(m_communicator)) {
                        ...
                        //        xout << bb0-bb.begin()<<std::endl;
                        {
                            TransitionDensity d(w,
                                                ...
                                                aa1,
                                                bb0, bb1,
                                                molpro::parityEven, true, !h.m_uhf);
                            if (nsb == bb.size())
...
//      xout << "number of alpha-alpha-excited strings=" << aa.size() << std::endl;
            ...
            for (unsigned int symb = 0; symb < 8; symb++) {
                ...
                for (StringSet::iterator aa1, aa0 = aa.begin();
                    ...
                    size_t nsa = aa1 - aa0;
                    TransitionDensity
                            d(w, aa0, aa1, w.betaStrings[symb].begin(), w.betaStrings[symb].end(),
...
molpro::Operator Wavefunction::density(int rank,
                                               bool uhf,
                                               bool hermitian,
        ...
        for (unsigned int syma = 0; syma < 8; syma++) {
            ...
            if (!NextTask(m_communicator) || nsa * nsb == 0) continue;
            {
                TransitionDensity d(*this,
                                    ...
                                    (hermitian ? molpro::parityEven : molpro::parityNone), true,
                                    !result.m_uhf);
//      xout << "Transition density\n"<<d<<std::endl;
                MXM(&((*result.O1(true).data())[0]),
                    &d[0],
                    &(bra->buffer[offset]),
                    orbitalSpace->total(0, (hermitian ? molpro::parityEven : molpro::parityNone)),
                    nsa * nsb,
                    1,
                    true,
                    nsa * nsb);
            ...
            if (result.m_uhf) {
                TransitionDensity d(*this,
                                    ...
                                    betaStrings[symb].begin(),
                                    betaStrings[symb].end(),
                                    (hermitian ? molpro::parityEven : molpro::parityNone), false, true);
        ...
        for (unsigned int syma = 0; syma < 8; syma++) {
            ...
            for (unsigned int symb = 0; symb < 8; symb++) {
                ...
                for (StringSet::iterator aa1, aa0 = aa.begin();
                    ...
                    size_t nsa = aa1 - aa0;
                    TransitionDensity
                            d(*this, aa0, aa1, betaStrings[symb].begin(), betaStrings[symb].end(),
                              molpro::parityOddPacked, false, false);
                    TransitionDensity
                            e(*bra, aa0, aa1, betaStrings[symb].begin(), betaStrings[symb].end(),
                              molpro::parityOddPacked, false, false);
        ...
        for (unsigned int symb = 0; symb < 8; symb++) {
            ...
            for (unsigned int syma = 0; syma < 8; syma++) {
                ...
                for (StringSet::iterator bb1, bb0 = bb.begin();
                    ...
                    size_t nsb = bb1 - bb0;
                    TransitionDensity
                            d(*this, alphaStrings[syma].begin(), alphaStrings[syma].end(), bb0, bb1,
                              molpro::parityOddPacked, false, false);
                    TransitionDensity
                            e(*bra, alphaStrings[syma].begin(), alphaStrings[syma].end(), bb0, bb1,
                              molpro::parityOddPacked, false, false);
...
molpro::vector<double>::iterator Wavefunction::begin() {
    return buffer.begin();
}

molpro::vector<double>::iterator Wavefunction::end() {
    return buffer.end();
}

molpro::vector<double>::const_iterator Wavefunction::cbegin() const {
    return buffer.cbegin();
}

molpro::vector<double>::const_iterator Wavefunction::cend() const {
    return buffer.cend();
}
