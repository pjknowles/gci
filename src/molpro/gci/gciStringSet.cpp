...
StringSet::StringSet(const StringSet &referenceSpace, int annihilations, int creations, int sym, bool parallel)
  ...
  //  xout << "StringSet constructor from referenceSpace size()="<<size()<<", rank="<<parallel_rank<<",
...
StringSet::StringSet(const std::vector<StringSet> &referenceSpaces, int annihilations, int creations, int sym,
  ...
  //  xout << "StringSet constructor from referenceSpaces size()=" << size() << ", rank=" << parallel_rank << ",
  //  parallel="
  //       << parallel << std::endl;
}
...
//  xout << "size="<<size()<<std::endl;
  ...
  if (parallel) {
    ...
    if (parallel_rank > 0) {
      ...
      //    xout <<"master size()="<<size()<<", bytestreamsize="<<serialised.size()/(size() ? size() : 1) <<",
...
void StringSet::addByOperators(const StringSet &referenceSpace, int annihilations, int creations, int sym,
  ...
  //  xout << "addByOperators parallel="<<parallel<<", parallel_rank="<<parallel_rank<<", count="<<count<<",
...
void StringSet::setupPartialWeightArray() { // set up partial weight array for addressing binomial distributions
  ...
  //    xout << "PartialWeightArray:"<<std::endl; for (int k=0;k<nitem;k++) { for (int l=0;l<nbox;l++) xout << "
  //    "<<PartialWeightArray[k][l]; xout <<std::endl; }
}
