...
TransitionDensity::TransitionDensity(const Wavefunction &w,
                                     ...
                                     const molpro::parity_t parity, const bool doAlpha, const bool doBeta)
        : molpro::array<double>(
...
//  xout << m_nsa << " " << m_nsb << " " << m_excitations << " " << m_nsa * m_nsb * m_excitations <<" = "<<size() << std::endl;
    ...
    if (m_deltaAlpha == 0 && m_deltaBeta == 0) { // number of electrons preserved, so one-electron excitation
            ...
            for (auto s = m_alphaStringsBegin; s != m_alphaStringsEnd; s++) {
                ...
                if (w.m_sparse) {
                    ...
                    for (const auto &e : ee) {
                        if (e.phase < 0)
                            for (size_t ib = 0; ib < m_nsb; ib++)
                                (*this)[offa + m_nsa * m_nsb * e.orbitalAddress + ib] -= w.buffer[woffset +
                                                                                                  e.stringIndex * wnsb +
                                                                                                  ib];
                        else
                            for (size_t ib = 0; ib < m_nsb; ib++)
            ...
            for (auto s = m_betaStringsBegin; s != m_betaStringsEnd; s++) {
                ...
                if (w.m_sparse) {
                    ...
                    for (const auto &e : ee) {
                        if (e.phase < 0)
                            for (size_t ia = 0; ia < m_nsa; ia++) {
                                *(v + m_nsa * m_nsb * e.orbitalAddress + ia * m_nsb) -= w.buffer[woffset +
                                                                                                 e.stringIndex +
                                                                                                 wnsb * ia];
        ...
        if (m_parity != molpro::parityNone) throw std::logic_error("wrong parity in alpha-beta");
        m_hasAlpha = true;
        m_hasBeta = true;
                                ...
                                for (const auto &ea : eea)
                                    if (w.buffer_sparse.count(woffset + eb.stringIndex + ea.stringIndex))
                                        (*this)[offb + eb.orbitalAddress + ea.orbitalAddress] +=
                                                ea.phase *
                                                w.buffer_sparse.at(woffset + eb.stringIndex + ea.stringIndex);
                            } else {
                                for (const auto &ea : eea)
                                    if (w.buffer_sparse.count(woffset + eb.stringIndex + ea.stringIndex))
                                        (*this)[offb + eb.orbitalAddress + ea.orbitalAddress] -=
...
molpro::Operator TransitionDensity::density(const Wavefunction &w) const {
    ...
    if (m_nsa * m_nsb != (w.blockOffset(syma + 1) - w.blockOffset(syma)))
        throw std::range_error(
                "Wrong dimensions, TransitionDensity::density"); // present implementation only for complete space in TransitionDensity
    MxmDrvTN(&((*result.O1(true).data())[0]),
             &this->at(0),
             &(w.buffer[w.blockOffset(syma)]),
             m_excitations,
             m_nsa * m_nsb,
             1,
             1);
