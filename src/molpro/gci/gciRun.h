...
private:
   ...
   * FCI \param maxOrder The maximum order of perturbation theory \param energyThreshold Convergence threshold for
   * energy \param maxIterations The maximum number of iterations to perform \return the energies order by order
   */
  std::vector<double> RSPT(const std::vector<molpro::Operator *> &hams, const State &prototype, int maxOrder = -1,
                           double energyThreshold = -1, int maxIterations = -1);
  std::vector<double> ISRSPT(const molpro::Operator &ham, const molpro::Operator &ham0, const State &prototype,
                             int maxOrder = -1, double energyThreshold = -1, int maxIterations = -1);
  void IPT(const molpro::Operator &ham, const State &prototype, size_t referenceLocation);
   ...
   */
  std::vector<double> Davidson(const molpro::Operator &ham, const State &prototype, double energyThreshold = (double)-1,
                               int nState = -1, int maxIterations = -1);
  std::vector<double> CSDavidson(const molpro::Operator &ham, const State &prototype,
   ...
   * \param prototype A State object specifying number of electrons, symmetry, spin
   * If it is a Wavefunction , then that will be used also for specifying the configuration space, otherwise the default
   * FCI \param energyThreshold Convergence threshold for energy \param maxIterations The maximum number of iterations
   * to perform \return the energy of the state.
   */
  std::vector<double> DIIS(const molpro::Operator &ham, const State &prototype, double energyThreshold = (double)-1,
                           int maxIterations = -1);
 ...
 */
void FCIDump(const molpro::Operator &op, const std::string filename,
