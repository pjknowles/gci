...
MixedOperatorSecondQuant::MixedOperatorSecondQuant(const Options &options)
    : m_fcidump_f(_fcidump_f(options)), hdf5_fname(_hdf5_fname(options)),
      restart(!options.parameter("HAM_HDF5_RESTART", "").empty()),
      hid_file(utils::open_hdf5_file(hdf5_fname, gci::mpi_comm_compute, !restart)), hdf5_file_owner(true),
      nMode(_nMode(options)), nModal(_nModal(options)), Hvib(nMode, nModal, parity_t::none, parity_t::even, "Hvib") {
  std::cout << "MixedOperatorSecondQuant: Loading the Hamiltonian" << std::endl;
  std::cout << "hdf5_fname = " << hdf5_fname << std::endl;
  if (restart)
    std::cout << "restarting" << std::endl;
  molpro::FCIdump fcidump(m_fcidump_f);
  if (options.parameter("POLARITONIC", 0)) {
    ...
    if (freq.empty())
      throw std::runtime_error("HO frequency not specified for polaritonic calculation");
    if (freq.size() > 1)
    ...
    if (gamma.empty())
      throw std::runtime_error("HO frequency not specified for polaritonic calculation");
    if (gamma.size() != 3)
      throw std::runtime_error("Polaritonic coupling must be a 3D vector");
    ...
    if (options.parameter("INCLUDE_LAMBDA", 0))
      initializeLambda(fcidump);
    if (options.parameter("INCLUDE_K", 0))
      initializeK(fcidump);
    if (options.parameter("INCLUDE_D", 0))
      initializeD(fcidump);
  }
}
...
inline PersistentOperator create_persistentoperator(const std::string &fcidump, bool restart, std::string description,
                                                    ...
                                                    Op (*construct_op)(const molpro::FCIdump &, bool)) {
  if (restart)
    return PersistentOperator(hid_file, description);
...
void MixedOperatorSecondQuant::initializeHel(const molpro::FCIdump &fcidump, bool h0_only) {
  int i_operator = 0;
  ...
  auto vibOp = VibOperator<hel_t>(nMode, nModal, parity_t::none, parity_t::even, name);
  for (int iMode = 0; iMode < nMode; ++iMode) {
    for (int iModal = 0; iModal < nModal; ++iModal) {
      for (int jModal = 0; jModal <= iModal; ++jModal) {
        f = fcidump.fileName() + "_Hel_" + std::to_string(iMode + 1) + "_" + std::to_string(iModal + 1) + "_" +
            std::to_string(jModal + 1);
        if (!utils::file_exists(f, "MixedOperatorSecondQuant::initializeHel(): fcidump not found, " + f))
          continue;
        VibExcitation vibExc({{iMode, iModal, jModal}});
        std::string description = name + "(" + std::to_string(iMode + 1) + "," + std::to_string(iModal + 1) + "," +
                                  std::to_string(jModal + 1) + ")";
        int root = i_operator % gci::parallel_size;
        ++i_operator;
        auto p_op = create_persistentoperator(f, restart, description, hid_file, root, constructOperator);
...
void MixedOperatorSecondQuant::initializeLambda(const molpro::FCIdump &fcidump) {
  std::string f, name = "Lambda[1]";
  auto vibOp = VibOperator<hel_t>(nMode, nModal, parity_t::none, parity_t::even, name);
  for (int iMode = 0; iMode < nMode; ++iMode) {
    for (int iModal = 0; iModal < nModal; ++iModal) {
      for (int jModal = 0; jModal < iModal; ++jModal) {
        f = fcidump.fileName() + "_Lambda_" + std::to_string(iMode + 1) + "_" + std::to_string(iModal + 1) + "_" +
            std::to_string(jModal + 1);
        ...
        std::string description = name + "(" + std::to_string(iMode + 1) + "," + std::to_string(iModal + 1) + "," +
                                  std::to_string(jModal + 1) + ")";
        auto op = std::make_shared<molpro::Operator>(constructOperatorAntisymm1el(molpro::FCIdump(f), true));
        op->m_description = description;
        auto p_op = PersistentOperator(op, description, 0, hid_file, true);
        vibOp.append(p_op, vibExc);
        if (iModal != jModal) {
          op = std::make_shared<molpro::Operator>(constructOperatorAntisymm1el(molpro::FCIdump(f), true) * (-1.));
...
void MixedOperatorSecondQuant::initializeK(const molpro::FCIdump &fcidump) {
  ...
  if (utils::file_exists(f, "MixedOperatorSecondQuant::initializeK(): fcidump not found, " + f)) {
    ...
    auto op = std::make_shared<molpro::Operator>(constructK(molpro::FCIdump(f), true));
    op->m_description = description;
    auto p_op = PersistentOperator(op, description, 0, hid_file, true);
  ...
  for (int iMode = 0; iMode < nMode; ++iMode) {
    for (int iModal = 0; iModal < nModal; ++iModal) {
      for (int jModal = 0; jModal <= iModal; ++jModal) {
        f = fcidump.fileName() + "_K_" + std::to_string(iMode + 1) + "_" + std::to_string(iModal + 1) + "_" +
            std::to_string(jModal + 1);
        if (!utils::file_exists(f, "MixedOperatorSecondQuant::initializeK(): fcidump not found, " + f))
          continue;
        VibExcitation vibExc({{iMode, iModal, jModal}});
...
void MixedOperatorSecondQuant::initializeD(const molpro::FCIdump &fcidump) {
  int i_operator = 0;
  ...
  for (int iMode = 0; iMode < nMode; ++iMode) {
    for (int iModal = 0; iModal < nModal; ++iModal) {
      for (int jModal = 0; jModal <= iModal; ++jModal) {
        f = fcidump.fileName() + "_D_" + std::to_string(iMode + 1) + "_" + std::to_string(iModal + 1) + "_" +
            std::to_string(jModal + 1);
        if (!utils::file_exists(f, "MixedOperatorSecondQuant::initializeD(): fcidump not found, " + f))
          continue;
        VibExcitation vibExc({{iMode, iModal, jModal}});
...
void constructHvib(VibOperator<double> &Hvib, const std::string &fcidump_name, int nmode, int nmodal) {
  molpro::FCIdump dump(fcidump_name + "_Hvib");
  dump.rewind();
  double value;
  int iMode, jModal, kModal, l;
  while (dump.nextIntegral(iMode, jModal, kModal, l, value) != molpro::FCIdump::endOfFile) {
    ...
    if (jModal > nmodal || kModal > nmodal)
      continue;
...
void constructDMcoupling(std::map<std::string, MixedOperatorSecondQuant::hel_t> &elHam2,
  ...
  assert(gamma.size() == 3);
  assert(nmode == 1);
  std::shared_ptr<molpro::Operator> D;
  auto xyz = std::vector<std::string>{"X", "Y", "Z"};
  for (size_t i = 0; i < 3; ++i) {
    auto f = fcidump_f + "_DM_" + xyz[i];
    auto dg = MixedOperatorSecondQuant::constructK(molpro::FCIdump(f), true);
    dg *= gamma[i];
    if (i == 0)
      D = std::make_shared<molpro::Operator>(dg);
    else
      *D += dg;
  }
  // elHam2 = (g.D) / sqrt(w); applied twice
  auto D2 = std::make_shared<molpro::Operator>(*D);
  *D2 *= 1. / std::sqrt(freq[0]);
  ...
  for (int i = 1; i < nmodal; ++i) {
    auto v = std::sqrt(i);
    auto Dnm = std::make_shared<molpro::Operator>(*D);
    *Dnm *= v;
...
molpro::Operator MixedOperatorSecondQuant::constructOperatorAntisymm1el(const molpro::FCIdump &dump, bool collective) {
  ...
  if (rank == 0 || !collective) {
    ...
    molpro::Operator result(molpro::dims_t{dim, dim, dim, dim}, 1, dump.parameter("IUHF")[0] > 0, {-1, -1}, {-1, -1}, 0,
                            true, false, "Hamiltonian Lambda[1]");
    result.zero();
...
molpro::Operator MixedOperatorSecondQuant::constructD(const molpro::FCIdump &dump, bool collective) {
  ...
  if (rank == 0 || !collective) {
    ...
    //        SymmetryMatrix::Operator result(dim, 2, dump.parameter("IUHF")[0] > 0, 0, true, "Hamiltonian");
    molpro::Operator result(molpro::dims_t{dim, dim, dim, dim}, 2, dump.parameter("IUHF")[0] > 0, {-1, -1}, {-1, -1}, 0,
                            true, false, "Hamiltonian D");
    result.zero();
