        ...
        for (int iModal = 0; iModal < nModal; ++iModal) {
            for (int jModal = 0; jModal < iModal; ++jModal) {
                ...
                std::string description = name + "(" + std::to_string(iMode + 1) + ","
                                          + std::to_string(iModal + 1) + ","
                                          + std::to_string(jModal + 1) + ")";
                auto op = std::make_shared<molpro::Operator>(
                        constructOperatorAntisymm1el(molpro::FCIdump(f), true));
                op->m_description = description;
                auto p_op = PersistentOperator(op, description, 0, hid_file, true);
                vibOp.append(p_op, vibExc);
                if (iModal != jModal) {
                    op = std::make_shared<molpro::Operator>(
                            constructOperatorAntisymm1el(molpro::FCIdump(f), true) * (-1.));
...
molpro::Operator MixedOperatorSecondQuant::constructOperatorAntisymm1el(const molpro::FCIdump &dump, bool collective) {
    ...
    if (rank == 0 || !collective) {
        ...
        molpro::Operator result(molpro::dims_t{dim, dim, dim, dim}, 1, dump.parameter("IUHF")[0] > 0,
...
//        SymmetryMatrix::Operator result(dim, 2, dump.parameter("IUHF")[0] > 0, 0, true, "Hamiltonian");
        molpro::Operator result(molpro::dims_t{dim, dim, dim, dim}, 2, dump.parameter("IUHF")[0] > 0,
