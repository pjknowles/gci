...
#include "molpro/gci/gciExcitationSet.h"
...
/*!
 * \brief The StringSet class holds a set of String objects, possibly the complete set for a given number of objects and
 * boxes
 */
class StringSet : public molpro::vector<String>, public Printable {
public:
  /*!
   ...
   * \param prototype A String object that will be copied into this as a source of the number of objects and boxes, and
   * symmetry information \param all Whether or not to construct the complete set of String objects \param sym If all,
   * specify symmetry of String objects; -1 denotes all symmetries
   */
   ...
   */
  explicit StringSet(const StringSet &referenceSpace, int annihilations, int creations, int sym = -1,
                     bool parallel = false);
  /*!
   ...
   * \param parallel whether to use parallel algorithm
   */
  explicit StringSet(const std::vector<StringSet> &referenceSpaces, int annihilations, int creations, int sym = -1,
                     bool parallel = false);
  /*!
  ...
  void insert(String &s) {
    ...
    if (addressMap.count(s.key())) {
      if (addressMap[s.key()] >= size())
        throw std::logic_error("something wrong in StringSet reset");
      at(addressMap[s.key()]) = s;
    } else {
      addressMap[s.key()] = size();
      molpro::vector<String>::push_back(s);
    }
  }
