...
size_t
OrbitalSpace::quadIndex(unsigned int i, unsigned int j, unsigned int k, unsigned int l, int parity, int parity2) const {
    unsigned int isym = orbital_symmetries[i - 1];
    unsigned int jsym = orbital_symmetries[j - 1];
...
std::string OrbitalSpace::str(int verbosity, unsigned int columns) const {
    ...
    if (verbosity >= 1) {
        ...
        for (std::vector<unsigned int>::const_iterator i = orbital_symmetries.begin(); i != orbital_symmetries.end();
             i++)
            o << " " << *i + 1;
    }
    return o.str();
