...
size_t OrbitalSpace::quadIndex(unsigned int i, unsigned int j, unsigned int k, unsigned int l, int parity,
                               int parity2) const {
  unsigned int isym = orbital_symmetries[i - 1];
  unsigned int jsym = orbital_symmetries[j - 1];
  ...
  return pairSpace.find(parity)->second.offset(isym ^ jsym ^ ksym ^ lsym, isym ^ jsym, parity2) +
         pairIndex(i, j, parity) + pairIndex(k, l, parity) * total(isym ^ jsym, parity);
}

unsigned int OrbitalSpace::orbitalIndex(unsigned int i) const {
  unsigned int n = 0;
  for (unsigned int j = 1; j < i; j++)
    if (orbital_symmetries[j - 1] == orbital_symmetries[i - 1])
      n++;
  return n;
}
