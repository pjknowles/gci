...
HProductSet::HProductSet(const HProduct &bra, const VibSpace &vibSpace, const VibOp &vibOp)
    : m_vibSpace(vibSpace), m_basis(), m_vibDim(0), m_excLvlDim(vibSpace.excLvl + 1, 0), m_connectedSet(true) {
  switch (vibOp.type) {
  case VibOpType::HO:
  ...
  case VibOpType::Q:
  case VibOpType::dQ:
    generateQcoupledSpace(bra, vibOp);
    break;
  case VibOpType::Qsq:
    generateQsqCoupledSpace(bra, vibOp);
    break;
  }
...
void HProductSet::setVibDim() {
  if (m_connectedSet)
    throw std::logic_error("Cannot be called for a connectedSet.");
...
size_t HProductSet::index(const HProduct &phi) const {
  ...
  if (phi.empty())
    ...
    throw std::logic_error("Index to more than doubly excited basis is not implemented yet.");
  return index;
}
...
void HProductSet::generateQcoupledSpace(const HProduct &bra, const VibOp &vibOp) {
  if (vibOp.type != VibOpType::Q && vibOp.type != VibOpType::dQ)
    throw std::logic_error("Mismatch of operator type");
