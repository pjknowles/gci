 ...
 */
enum class VibOpType { HO, Q, dQ, Qsq };
...
public:
  VibOp(const VibOpType &type, const std::vector<unsigned int> &mode = {}) : type(type), mode(mode) {
    ...
    case VibOpType::Q:
      if (mode.size() != 1)
        throw std::logic_error("Q operator is one-dimensional");
      break;
    case VibOpType::dQ:
      if (mode.size() != 1)
        throw std::logic_error("dQ operator is one-dimensional");
      break;
    case VibOpType::Qsq:
      if (mode.size() != 2)
        throw std::logic_error("Qsq operator is two-dimensional");
      break;
    default:
 ...
 */
struct MixedOpTerm {
  MixedOpTerm(VibOp vibOp, const molpro::FCIdump &fcidump) : vibOp(std::move(vibOp)), Hel(constructOperator(fcidump)){};

  MixedOpTerm(VibOp vibOp, const molpro::Operator &op) : vibOp(std::move(vibOp)), Hel(op){};
  VibOp vibOp;
  molpro::Operator Hel;
};
...
public:
   ...
   */
  explicit MixedOperator(const molpro::FCIdump &fcidump);

  MixedOperator()
      : nMode(0), freq({}), zpe(0), Hel({0, 0}, 0, false, 0, true, false, "dummy"), Hmix({}), m_inc_d1(false),
  ...
  molpro::Operator Hel;                               //!< Electronic Hamiltonian
