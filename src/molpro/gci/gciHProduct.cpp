...
bool HProduct::withinSpace(const VibSpace &vibSpace) const {
  if (excLvl() > vibSpace.excLvl)
    return false;
  return !std::any_of(m_prod.cbegin(), m_prod.cend(), [&vibSpace](const modal_t &modal) {
    return modal[0] >= vibSpace.nMode || modal[1] >= vibSpace.nModal || modal[1] < 0;
  });
}

void HProduct::changeModal(const int iMode, const int diff) {
  auto modal = std::find_if(m_prod.begin(), m_prod.end(), [&iMode](const modal_t &el) { return (el[0] == iMode); });
  if (modal == m_prod.end()) {
