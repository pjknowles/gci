...
ExcitationSet::ExcitationSet(const String &from, const StringSet &to, int annihilations, int creations,
                             molpro::parity_t parity)
    : From(from), To(to) {
  constexpr String::orbital_type one = 1;
  int symexc = -1;
  //    cout <<"ExcitationSet taking From="<<From.str()<<" annihilations="<<annihilations<<"
  //    creations="<<creations<<std::endl;
  if (to.symmetry >= 0)
    symexc = from.symmetry ^ static_cast<unsigned int>(to.symmetry); // use symmetry if we can
  ...
  if (annihilations + creations == 1) {
    int ii = 0;
    buffer.reserve(from.orbitalSpace->orbital_symmetries.size());
    ...
    //            cout << "two-orbital excitation, parity="<<parity<<std::endl;
    ...
    for (String::orbital_type j = 0; j < (int)from.orbitalSpace->orbital_symmetries.size(); j++) {
      ...
      if (phasea) {
        for (String::orbital_type i = 0;
             i < (int)(annihilations == creations ? from.orbitalSpace->orbital_symmetries.size() : j); i++) {
          if (from.orbitalSpace->orbital_symmetries[i] ==
            ...
            if (phase) {
              ...
              size_t ti = to.addressMap.find(tt.key())->second;
              if (ti >= to.size()) {
                cout << "i=" << i + one << " phase=" << phase << " ti=" << ti << " tt=" << tt.str() << std::endl;
                throw std::range_error("index error in ExcitationSet");
              }
              if (from.orbitalSpace->orbital_symmetries[i] > from.orbitalSpace->orbital_symmetries[j])
                phase = -phase;
...
std::string ExcitationSet::str(int verbosity) const {
  if (To.empty() || verbosity < 0)
    return "";
  std::stringstream ss;
  ss << "Excitations for String " << From.str() << " into symmetry " << To[0].symmetry + 1 << ":";
  for (const auto &e : buffer) {
    ss << std::endl
       << " String index=" << e.stringIndex << "(" << To[e.stringIndex].str() << ")"
