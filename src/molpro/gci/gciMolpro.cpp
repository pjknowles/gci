...
#endif
      if (eigen) {
    Eigen::Map<Eigen::MatrixXd, Eigen::Unaligned, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>> Am(
        const_cast<double *>(A), nRows, nLink, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>(nRows, 1));
    Eigen::Map<Eigen::MatrixXd, Eigen::Unaligned, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>> Bm(
        const_cast<double *>(B), nLink, nCols, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>(nCols, 1));
    Eigen::Map<Eigen::MatrixXd, Eigen::Unaligned, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>> Outm(
        Out, nRows, nCols, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>(nRows, 1));
    if (AddToDest)
      Outm += Am * Bm;
    else
      Outm = Am * Bm;
    ...
    for (uint s = 0; s < nCols; s++)
      for (uint r = 0; r < nRows; r++)
        for (uint t = 0; t < nLink; t++)
          Out[r + s * nRows] += A[r + t * nRows] * B[t + s * nLink];
  }
}

void MxmDrvTN(double *Out, const double *A, const double *B, uint nRows, uint nLink, uint nStrideLink, uint nCols,
              bool AddToDest) {
#ifdef EIGEN_USE_MKL_ALL
  if (blas) {
    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, nRows, nCols, nLink, 1, A, nStrideLink, B, nLink,
                AddToDest ? 1 : 0, Out, nRows);
  } else
#endif
      if (eigen) {
    ...
    Eigen::Map<Eigen::MatrixXd, Eigen::Unaligned, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>> Am(
        const_cast<double *>(A), nRows, nLink, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>(1, nStrideLink));
    Eigen::Map<Eigen::MatrixXd, Eigen::Unaligned, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>> Bm(
        const_cast<double *>(B), nLink, nCols, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>(1, nCols));
    Eigen::Map<Eigen::MatrixXd, Eigen::Unaligned, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>> Outm(
        Out, nRows, nCols, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>(nRows, 1));
    //      if (nCols*nLink) {
    ...
    for (uint s = 0; s < nCols; s++)
      for (uint r = 0; r < nRows; r++)
        for (uint t = 0; t < nLink; t++)
          Out[r + s * nRows] += A[r * nStrideLink + t] * B[t + s * nLink];
  }
}

void MxmDrvGen(double *Out, uint nRowStOut, uint nColStOut, const double *A, uint nRowStA, uint nColStA,
               const double *B, uint nRowStB, uint nColStB, uint nRows, uint nLink, uint nCols, bool AddToDest) {
  if (!AddToDest)
    for (uint s = 0; s < nCols; s++)
      for (uint r = 0; r < nRows; r++)
  ...
  for (uint s = 0; s < nCols; s++)
    for (uint r = 0; r < nRows; r++)
      for (uint t = 0; t < nLink; t++)
        Out[r * nRowStOut + s * nColStOut] += A[r * nRowStA + t * nColStA] * B[t * nRowStB + s * nColStB];
}
